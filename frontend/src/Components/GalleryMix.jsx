  ...
  return (
    <div className="w-full">
      <div className="relative right-0">
        {/* Tab section */}
        <div className="w-full">
          <ul className="font-body uppercase relative flex flex-wrap px-1.5 py-1.5 list-none">
            {["Seniors", "Couples", "Family"].map((tab) => (
              <li
                key={tab}
                className={`z-30 flex-auto text-center mx-1 cursor-pointer rounded-md ${
                  activeTab === tab
                    ? "text-black font-semibold bg-lgrey"
                    : "text-black bg-lilac"
                }`}
                onClick={() => handleTabClick(tab)}
              >
                <a
                  className={`z-30 flex items-center justify-center w-full px-0 py-2 text-sm mb-0 transition-all ease-in-out border-2 rounded-md ${
                    activeTab === tab
                      ? "border-transparent"
                      : "border-transparent"
                  } hover:border-white`}
